...
public class Binaria extends Busquedas implements MOrdenamiento {
     ...
     */
    public static Integer Busqueda(int[] v, int n) {//MÃ©todo para buscar binariamente
    ...
    public void MSeleccion(int[] a) {
        for (int i = 0; i < a.length - 1; i++) {
            for (int j = i + 1; j < a.length; j++) {//Arregla los elementos comparando el nÃºmero de la casilla con los demÃ¡s siguientes
                if (a[j] < a[i]) {
                    int au = a[j];
                    a[j] = a[i];
                    a[i] = au;
